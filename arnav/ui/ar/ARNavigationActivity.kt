...
class ARNavigationActivity : AppCompatActivity() {
    ...
    // ── Camera-locked HUD arrow (direction_arrow.glb) ────────────────────────
    ...
    // ── NavigationService: track last sent instruction to avoid redundant calls
    ...
    companion object {
        ...
        // ── HUD arrow sizing (direction_arrow.glb locked to camera) ──────────
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // --- Request "Draw Over Other Apps" Permission for the Floating HUD ---
    ...
    private fun checkLoadingStatus() {
        if (isArLoaded && isLocationFound) {
            // Both are ready! Fade out the loading screen smoothly.
            runOnUiThread {
                binding.loadingOverlay.animate()
                    .alpha(0f)
                    .setDuration(400) // 0.4 seconds fade out
                    .withEndAction {
    ...
    override fun onResume() {
        ...
        // FIX: Use startForegroundService to prevent Android 8+ crashes
        val hideIntent = android.content.Intent(this, com.campus.arnav.service.NavigationService::class.java)
        hideIntent.action = com.campus.arnav.service.NavigationService.ACTION_HIDE_OVERLAY
    ...
    override fun onUserLeaveHint() {
        ...
        if (android.provider.Settings.canDrawOverlays(this)) {
            // FIX: Use startForegroundService to prevent Android 8+ crashes
            val showIntent = android.content.Intent(this, com.campus.arnav.service.NavigationService::class.java)
            showIntent.action = com.campus.arnav.service.NavigationService.ACTION_SHOW_OVERLAY
    ...
    private fun updateARNodesPosition() {
        ...
        routeArrows.forEachIndexed { index, arrow ->
            ...
            if (index in passedArrowIndices) return@forEachIndexed
        ...
        routeArrows.forEachIndexed { index, arrow ->
            ...
            if (arrow.isDestination) {
                ...
                node.worldRotation = Rotation(
                    x = 0f,
                    y = (System.currentTimeMillis() % 3600) / 10f,
                    z = 0f
                )
                return@forEachIndexed
            }
        ...
        // ── 3. Update the camera-locked HUD arrow ────────────────────────────
    ...
    // ════════════════════════════════════════════════════════════════════════
    // HUD ARROW  (direction_arrow.glb — upright billboard, rotates to point
    // at the nearest upcoming route node — matches screenshot style)
    // ════════════════════════════════════════════════════════════════════════
    ...
    private fun updateHudArrow(
        ...
        val camYaw = cam.worldRotation.y
        ...
        val bearingToNode = userLoc.bearingTo(targetLoc)
        var relativeAngle = bearingToNode - currentHeading
        while (relativeAngle < -180f) relativeAngle += 360f
        while (relativeAngle >  180f) relativeAngle -= 360f

        val worldAngle = relativeAngle + camYaw

        // --- FIXED: x = 0f lays the arrow flat so it acts like a compass needle pointing left/right ---
        ...
        val distanceText = "%.0fm".format(userLoc.distanceTo(targetLoc))

        // --- FIXED: Determine turn direction and pass it to the service ---
        var directionCode = "straight"
        val instruction = when {
            relativeAngle > 20f  -> { directionCode = "right"; "Turn right" }
            relativeAngle < -20f -> { directionCode = "left"; "Turn left" }
            else                 -> { directionCode = "straight"; "Continue straight" }
        }
    ...
    private fun updateNavigationService(instruction: String, distance: String, directionCode: String) {
        ...
        val intent = android.content.Intent(
            this, com.campus.arnav.service.NavigationService::class.java
        )
        intent.action = com.campus.arnav.service.NavigationService.ACTION_UPDATE
        intent.putExtra(com.campus.arnav.service.NavigationService.EXTRA_INSTRUCTION, instruction)
        intent.putExtra(com.campus.arnav.service.NavigationService.EXTRA_DISTANCE, distance)
        intent.putExtra(com.campus.arnav.service.NavigationService.EXTRA_DIRECTION, directionCode) // Sends the icon code!
    ...
    private fun startSensors() {
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(result: LocationResult) {
                ...
                binding.miniMapView.controller.setCenter(
